use memmap2::Mmap;
use crate::telemetry::{SharedMemoryObjectOut, update_telemetry};
use tauri::{State};
use serde::Serialize;
...
#[tauri::command]
pub fn is_last_best(state: State<'_, MmapState>, tele_type: String, car_num: usize) -> Result<bool, String> {
    let mmap = &state.mmap;
...
#[tauri::command]
pub fn get_lap(state: State<'_, MmapState>, tele_type: String, car_num: usize) -> Result<i32, String> {
    let mmap = &state.mmap;
    let telemetry = update_telemetry(mmap).ok_or_else(|| "TelemetryReadFailed".to_string())?;
    let tele_type = GraphViewDataType::from_string(&tele_type, car_num);
...
#[tauri::command]
pub fn get_values(state: State<'_, MmapState>, tele_type: String, car_num: usize) -> Result<DataPointResponse, String> {
    let mmap = &state.mmap;
    let telemetry = update_telemetry(mmap).ok_or_else(|| "TelemetryReadFailed".to_string())?;
    let tele_type = GraphViewDataType::from_string(&tele_type, car_num);
...
#[tauri::command]
pub fn get_context(state: State<'_, MmapState>, tele_type: String, car_num: usize) -> TelemetryContext {
    let mmap = &state.mmap;
